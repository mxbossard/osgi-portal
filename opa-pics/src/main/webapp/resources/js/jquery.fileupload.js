/*
 * jQuery File Upload Plugin 5.32.6 https://github.com/blueimp/jQuery-File-Upload Copyright 2010, Sebastian Tschan
	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($) {
			...
			'(Android (1\\.[0156]|2\\.[01]))' + '|(Windows Phone (OS 7|8\\.0))|(XBLWP)|(ZuneWP)|(WPDesktop)'
					+ '|(w(eb)?OSBrowser)|(webOS)' + '|(Kindle/(1\\.0|2\\.[05]|3\\.0))')
					...
					.widget(
							'blueimp.fileupload',
							{
								...
								options : {
									...
									add : function(e, data) {
										if (data.autoUpload
												|| (data.autoUpload !== false && $(this).fileupload('option',
														'autoUpload'))) {
								...
								_onProgress : function(e, data) {
									if (e.lengthComputable) {
										...
										if (data._time && data.progressInterval
												&& (now - data._time < data.progressInterval) && e.loaded !== e.total) {
											return;
										}
										data._time = now;
											...
											if (options.blob) {
												...
												$.each(options.files, function(index, file) {
													...
													if (that._isInstanceOf('File', file)
														...
														formData.append(options.paramName[index] || paramName, file,
																file.name);
													}
												});
											}
								...
								_addConvenienceMethods : function(e, data) {
									...
									data.submit = function() {
										if (this.state() !== 'pending') {
											data.jqXHR = this.jqXHR = (that._trigger('submit', e, this) !== false)
													&& that._onSend(e, this);
										}
								...
								_getUploadedBytes : function(jqXHR) {
									var range = jqXHR.getResponseHeader('Range'), parts = range && range.split('-'), upperBytesPos = parts
								...
								_chunkedUpload : function(options, testOnly) {
									...
									var that = this, file = options.files[0], fs = file.size, ub = options.uploadedBytes, mcs = options.maxChunkSize
											|| fs, slice = this._blobSlice, dfd = $.Deferred(), promise = dfd.promise(), jqXHR, upload;
									...
									upload = function() {
										...
										jqXHR = ((that._trigger('chunksend', null, o) !== false && $.ajax(o)) || that
												._getXHRPromise(false, o.context)).done(
								...
								_onSend : function(e, data) {
									...
									var that = this, jqXHR, aborted, slot, pipe, options = that._getAJAXSettings(data), send = function() {
										...
										jqXHR = jqXHR
												|| (((aborted || that._trigger('send', e, options) === false) && that
														...
														.always(
																function(jqXHRorResult, textStatus, jqXHRorError) {
																	...
																	if (that._active === 0) {
																		// The stop callback is triggered when all
																		// uploads have
																		// been completed, equivalent to the global
																		// ajaxStop event:
																		that._trigger('stop');
																	}
																});
								...
								_onAdd : function(e, data) {
									var that = this, result = true, options = $.extend({}, this.options, data), limit = options.limitMultiFileUploads, paramName = this
								...
								_handleFileTreeEntry : function(entry, path) {
									...
									if (entry.isFile) {
										...
										dirReader.readEntries(function(entries) {
											that._handleFileTreeEntries(entries, path + entry.name + '/').done(
													function(files) {
														dfd.resolve(files);
													}).fail(errorHandler);
										}, errorHandler);
								...
								_getFileInputFiles : function(fileInput) {
									...
									return $.when.apply($, $.map(fileInput, this._getSingleFileInputFiles)).pipe(
											function() {
												return Array.prototype.concat.apply([], arguments);
											});
								},
								...
								_onDragOver : function(e) {
									e.dataTransfer = e.originalEvent && e.originalEvent.dataTransfer;
									var dataTransfer = e.dataTransfer, data = {
										dropEffect : 'copy',
										preventDefault : true
									};
