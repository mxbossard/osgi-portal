/*
 * jQuery File Upload AngularJS Plugin 1.4.5 https://github.com/blueimp/jQuery-File-Upload Copyright 2013, Sebastian
 * Tschan https://blueimp.net Licensed under the MIT license: http://www.opensource.org/licenses/MIT
 */
...
(function(factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		define(['jquery', 'angular', './jquery.fileupload-image', './jquery.fileupload-audio',
	...
	.provider(
			...
			function() {
				var scopeApply = function() {
					var scope = angular.element(this).fileupload('option', 'scope')(), $timeout = angular.injector(
							['ng']).get('$timeout');
					...
					add : function(e, data) {
						...
						data.process(function() {
							return scope.process(data);
								...
								function() {
									var file = data.files[0], submit = function() {
										return data.submit();
									};
									...
									scope.$apply(function() {
										...
										if (file.$submit && (scope.option('autoUpload') || data.autoUpload)
					...
					function($scope, $element, $attrs, $window, fileUpload) {
						...
						$element.fileupload(angular.extend({
							...
							data.scope = $scope.option('scope');
								...
								['fileuploadadd', 'fileuploadsubmit', 'fileuploadsend', 'fileuploaddone',
										'fileuploadfail', 'fileuploadalways', 'fileuploadprogress',
										'fileuploadprogressall', 'fileuploadstart', 'fileuploadstop',
										'fileuploadchange', 'fileuploadpaste', 'fileuploaddrop', 'fileuploaddragover',
										'fileuploadchunksend', 'fileuploadchunkdone', 'fileuploadchunkfail',
										'fileuploadchunkalways', 'fileuploadprocessstart', 'fileuploadprocess',
										'fileuploadprocessdone', 'fileuploadprocessfail', 'fileuploadprocessalways',
										'fileuploadprocessstop'].join(' '), function(e, data) {
	...
	.controller('FileUploadPreviewController',
			['$scope', '$element', '$attrs', '$parse', function($scope, $element, $attrs, $parse) {
				var fn = $parse($attrs.fileUploadPreview), file = fn($scope);
				if (file.preview) {
			...
			function() {
				return function(scope, elm) {
					elm.on('dragstart', function(e) {
						try {
							e.originalEvent.dataTransfer.setData('DownloadURL', ['application/octet-stream',
									elm.prop('download'), elm.prop('href')].join(':'));
						} catch (ignore) {
						}
					});
				};
